...
RAW = ROOT / "research_workspace" / "05_data" / "raw"
INTERIM = ROOT / "research_workspace" / "05_data" / "interim"
PROCESSED = ROOT / "research_workspace" / "05_data" / "processed"
SAMPLING = ROOT / "research_workspace" / "01_sampling_frame"
REGISTRY = ROOT / "research_workspace" / "03_platform_registry"
...
def normalize_verified_items() -> pd.DataFrame:
    ...
    # Harmonize expected source columns.
    for col in [
...
def load_sampling_status() -> pd.DataFrame:
    # Prefer consolidated screening files if available. Multiple batches are allowed.
    ...
    out = out.drop_duplicates("org_id", keep="last")
    return out
...
def derive_analysis_inclusion(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Known duplicate alias from final reconciliation.
    df.loc[df["org_id"].eq("TC083"), "analysis_inclusion_status"] = "duplicate_alias"

    # Program-level cases are retained but marked for restricted use if sampling notes indicate this.
...
def load_interaction_aggregates() -> pd.DataFrame:
    ...
    # Uses provisional precode/boolean fields present in current interaction files.
    rows = []
    for item_id, g in i.groupby("item_id", dropna=False):
        ...
        def cnt(*terms: str) -> int:
            return sum(any(t in (str(r).lower()) for t in terms) for r in combined.split("\n"))
        rows.append({
            "item_id": item_id,
...
def load_recirculation() -> pd.DataFrame:
    ...
    if count_col:
        out = r[["item_id", count_col]].copy()
        out = out.rename(columns={count_col: "observed_recirculation_minimum"})
        return out.drop_duplicates("item_id", keep="last")
    # If each row is one observed trace, aggregate row count.
...
def main() -> None:
    ...
    # Strict primary corpus: verified, in study window, final included/program-level units only.
    strict = items[
    ...
    # Preserve all verified collection history separately.
